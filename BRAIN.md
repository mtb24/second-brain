...
MCP_SECRET=...
ANTHROPIC_API_KEY=...
DIGITALOCEAN_MODEL_ACCESS_KEY=...   ← DigitalOcean Serverless Inference model access key for Cortex/OpenClaw
COINGECKO_API_KEY=...
...
- Agent: **Cortex 🧠** — live Telegram/gateway routing is **DigitalOcean Serverless Inference first** (2026-04-25): primary **`digitalocean/openai-gpt-5.4-mini`**, fallbacks **`ollama/qwen2.5:0.5b`** then **`ollama/llama3.2-1b-16k:latest`**. The DigitalOcean provider is configured as OpenAI-compatible (`baseUrl: https://inference.do-ai.run/v1`, `api: openai-completions`, `compat.supportsStore: false`) with `DIGITALOCEAN_MODEL_ACCESS_KEY` in **`~/.openclaw/.env`**, loaded by the systemd drop-in **`~/.config/systemd/user/openclaw-gateway.service.d/env.conf`**. This keeps Cortex off Anthropic models while retaining the DigitalOcean route. *(Tournament Docker bots are separate — they may still use Anthropic via **`TOURNAMENT_*`** until migrated.)*
- Telegram bot: `@kensbrainbot`, Ken telegramUserId: `7221971575`
- **Ollama on brain VPS (2026-04-07)** — upgraded to **`0.20.2`** (was **`0.18.0`**). Older builds return **412** when pulling **Gemma 4** (“requires a newer version of Ollama”).
- **Gemma 4 + Ollama (Google partnership)** — Official edge tag **`gemma4:e2b`** ([ollama.com/library/gemma4](https://ollama.com/library/gemma4)): ~**7.2 GB** on disk, **`gemma4.context_length` = 131072** in **`curl …/api/show`** → satisfies OpenClaw’s **≥16000** context **without** a custom Modelfile. **Not usable on the current 4GB droplet:** runtime check returned **`model requires more system memory (7.3 GiB) than is available (3.9 GiB)`** from **`/api/generate`**. Treat **`gemma4:e2b`** as the default **smallest** Gemma 4 variant to try on a **~8GB+ RAM** (or remote) Ollama host; on **4GB**, keep **`llama3.2-1b-16k`** (or similar) if it fits RAM+swap.
...
- **OpenClaw** on VPS: **`2026.4.23`** (2026-04-24). Keep Mission Control’s gateway **`connect` `client.version`** in [mission-control/app/server/openclawGateway.ts](mission-control/app/server/openclawGateway.ts) aligned with **`openclaw --version`**. History: 2026-04-01 `2026.3.13` → `2026.3.28` → `2026.4.2` → `2026.4.23`.
...
- **Cortex: prior Gemini quota + Groq TPM + Ollama RAM chain** — Logs may show **`All models failed`** chaining **Google 429** (quota/billing), **Groq** **`413 Request too large … TPM`** on huge sessions, then **Ollama** local failures on the **4GB** VPS. **Current mitigation (2026-04-24):** DigitalOcean Serverless Inference is the primary route, with local Ollama degraded fallbacks. If this regresses, verify `DIGITALOCEAN_MODEL_ACCESS_KEY` in `~/.openclaw/.env`, `models.providers.digitalocean`, and the `/v1/models` endpoint before reintroducing Groq.
