...
- Agent: **Cortex 🧠** — **no Anthropic** on the Telegram/gateway path (2026-04-07): primary **`google/gemini-2.5-flash`**, fallbacks **Groq** then **Ollama**; keys in **`~/.openclaw/.env`**, plus systemd drop-in **`~/.config/systemd/user/openclaw-gateway.service.d/env.conf`** with **`EnvironmentFile=-/home/brain/.openclaw/.env`** so the gateway process always inherits **`GEMINI_API_KEY`** and **`GROQ_API_KEY`**. The primary id must exist in the **bundled model catalog** (e.g. **`google/gemini-2.5-flash`**); preview aliases can disappear on upgrade and cause Telegram’s generic *“Something went wrong…”* while **`journalctl --user -u openclaw-gateway`** shows **`Unknown model: …`**. *(Tournament Docker bots are separate — they may still use Anthropic via **`TOURNAMENT_*`** until migrated.)*
...
- **Cortex: Gemini quota + Groq TPM + Ollama RAM** — Logs may show **`All models failed`** chaining **Google 429** (quota/billing), **Groq** **`413 Request too large … TPM`** on huge sessions (free tier token-per-minute limits; OpenClaw may label this as **`rate_limit`**), then **Ollama** **`model requires more system memory`** on **4GB** RAM at **16k** ctx. **Mitigations:** fix **Gemini** quota or use a **new session** (**`/new`**) so Groq is not sent an oversized request; optional **swap** or **more RAM** if you need Ollama fallback to ever load. **Live routing (2026-04-07):** fallbacks **`groq/llama-3.1-8b-instant`**, **`groq/llama-3.3-70b-versatile`**, **`ollama/llama3.2-1b-16k:latest`** — set with **`openclaw models --agent main fallbacks …`** after exporting keys from **`~/.openclaw/.env`** in the shell (CLI), then **`systemctl --user restart openclaw-gateway`**.
