...
- **Cortex Telegram generic error when Gemini is rate-limited and Ollama cannot load 16k context** — **`journalctl --user -u openclaw-gateway`** may show **`Embedded agent failed before reply: All models failed …`** with **`429`** on **`google/gemini-2.5-flash`** and **`model requires more system memory …`** on **`ollama/llama3.2-16k`** (3B + large KV cache). Even **`llama3.2-1b-16k`** can still fail on a **4GB** VPS when free RAM is tight (~**1.6 GiB** available vs ~**1.7 GiB** requested). **Fix (live on VPS 2026-04-07):** insert a **cloud** fallback **before** Ollama — e.g. **`openclaw models --agent main fallbacks clear`**, then **`openclaw models --agent main fallbacks add anthropic/claude-3-5-haiku-20241022`**, then add **`ollama/llama3.2-1b-16k:latest`** and **`ollama/llama3.2-16k:latest`** if you still want local last-resort; **`systemctl --user restart openclaw-gateway`**. Requires **Anthropic** API key in OpenClaw auth (already present for **`main`**). Still restore **Gemini quota/billing** so the primary model is usable.
